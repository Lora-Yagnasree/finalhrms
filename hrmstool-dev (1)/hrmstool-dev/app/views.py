...
def loginview(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Don't reveal if user exists — show generic error later
            pass

        # ✅ Handle account lock if user exists
        if user_obj and hasattr(user_obj, "check_lock_status") and user_obj.check_lock_status():
            return render(
                request,
                "send_unlock_request.html",
                {"user": user_obj, "company_id": company_id}
            )
        ...
        # 🚫 Wrong credentials / company mismatch / user not found
        if user_obj and hasattr(user_obj, "failed_attempts"):
...
def unlock_user(request, request_id):
    ...
    message = f"Hello {user_obj.first_name},\n\n" \
          "We are pleased to inform you that your account has been successfully unlocked by our HR/Manager team. " \
          "You can now access your account and resume your activities without any restrictions.\n\n" \
          "If you did not request this action or believe this was done in error, please contact our support team immediately.\n\n" \
          "For any login issues or further assistance, feel free to reach out to us.\n\n" \
          "Regards,\n" f"{unlock_request.user.company}"
...
def search_results(request):
    ...
    page_urls = {
        ...
        'resignation request': 'resignation_requests',
        #help desk 
        'help desk': 'help_desk',
        'help desk ticket': 'help_desk',
        'support ticket': 'help_desk',
        'it support': 'help_desk',
        'it support ticket': 'help_desk',
        'it help desk': 'help_desk',
        'asset support': 'help_desk',
        'asset support ticket': 'help_desk',
        'asset help desk': 'help_desk',
        'hr support': 'help_desk',
        'hr support ticket': 'help_desk',
        'hr help desk': 'help_desk',
        # unlock 
        'unlock': 'unlock_requests',
        # employee self service
        'employee self service': 'employee_self_service',
        # career development 
        'career development': 'career_development',
        'skills' : 'career_development',
        'edit resource': 'career_development',
        'delete resource': 'career_development',
        'delete category': 'career_development',
        'resource detail': 'career_development',

        # Teams
        'teams': 'list_teams',
        'history': 'hr_login_logs',
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            f"OTP: {otp}\n\n"
            f"PLEASE NOTE: This OTP is valid for the next 10 minutes.\n"
            f"If you did not request a password reset, kindly ignore this message or contact our support team immediately.\n\n"
            f"Best regards,\n"
            f"HRMS Team",
...
def review_muster_notifications(request, muster_id, action):
    ...
    return JsonResponse({"status": muster_request.status, "id": muster_request.id})
...
def review_leaves_notifications(request, leaves_id, action):
    ...
    return JsonResponse({"status": leave_request.status, "id": leave_request.id})
...
def review_expense_notifications(request, expense_id, action):
    ...
    if action == 'Approve':
        ...
        expense_claim.save()
        message = f"Your expense claim (Bill No: {expense_claim.bill_no}, Amount: ₹{expense_claim.amount}) has been approved."
        Notification.objects.create(recipient=expense_claim.employee, message=message)

    elif action == 'Reject':
        expense_claim.status = 'Rejected'
        expense_claim.save()
        message = f"Your expense claim (Bill No: {expense_claim.bill_no}, Amount: ₹{expense_claim.amount}) has been rejected."
        Notification.objects.create(recipient=expense_claim.employee, message=message)

    return JsonResponse({"status": expense_claim.status, "id": expense_claim.id})
...
def review_loan_notifications(request, loan_id, action):
    ...
    if action == 'Approve':
        ...
        loan_request.save()
        message = f"Your loan request ({loan_request.loan_type}, Amount: ₹{loan_request.loan_amount}) has been approved."
        Notification.objects.create(recipient=loan_request.employee, message=message)

    elif action == 'Reject':
        loan_request.status = 'Rejected'
        loan_request.save()
        message = f"Your loan request ({loan_request.loan_type}, Amount: ₹{loan_request.loan_amount}) has been rejected."
        Notification.objects.create(recipient=loan_request.employee, message=message)

    return JsonResponse({"status": loan_request.status, "id": loan_request.id})
...
def user_delete(request, user_id):
    if request.method == 'POST':
        ...
        user.delete()
        # Redirect after deletion (update URL as needed)
        return redirect('user_list')
#------------------------------------------------------------- Policies #
...
def delete_salary(request, salary_id):
    if request.method == 'POST':
        ...
        salary.delete()
        return redirect('salary_list')  # Your salary list url name
        ...
        if to_user and to_user.email:
            send_mail(
                ...
                message=(
                    ...
                    f"A new support ticket has been raised and assigned to you for review.\n\n"
                    f"Ticket Details:\n"
                    f"- Raised by: {ticket.employee}\n"
                    f"- Category: {ticket.get_category_display()}\n"
                    f"- Issue Type: {ticket.issue_type}\n\n"
                    f"Description:\n{ticket.description}\n\n"
                    f"Please review and take the necessary action at your earliest convenience.\n\n"
                    f"Best regards,\n"
                    f"{user_employee.company}"
                ),
...
def review_resignation_request(request, resignation_id, action):
    ...
    if action_lower == 'approve':
        ...
        Notification.objects.create(recipient=resignation.employee, message=response_message)
...
def career_development(request):
    categories = SkillCategory.objects.filter(company=request.user.company)
    ...
    # limit the category choices to the user's company
    resource_form.fields['category'].queryset = SkillCategory.objects.filter(company=request.user.company)
 
    if request.method == 'POST' and can_edit:
        if 'add_resource' in request.POST:
            resource_form = CareerResourceForm(request.POST, request.FILES)
            resource_form.fields['category'].queryset = SkillCategory.objects.filter(company=request.user.company)  # 👈 again on POST
...
def edit_resource(request, slug):
    ...
    if request.method == 'POST':
        ...
        form.fields['category'].queryset = SkillCategory.objects.filter(company=request.user.company)  # 👈 filter here
        if form.is_valid():
            form.save()
            return redirect('career_development')
    else:
